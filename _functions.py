...
class Functions:
    ...
    def function_args(self, tree):
        ...
        for child in tree.children:
            if isinstance(child, Tree):
                # Modified
                if child.children[0].data == "named_parameter":
                    #child.children[0].children contains 2 elements if child.children[0] is a named parameter
                    #If it's SUGGESTION EQUALS TRUE, child.children[0].children = [SUGGESTION, TRUE]
                    #We want to parse if like a single constant, so we remove SUGGESTION
